...
4b. **設定値の意味をフィールド名から推測しない。**
   画面が「各設定の意味」「配線状況」を申告していればそれが正本。無ければ
   「この値の意味はこの画面からは判断できない」と述べる。
   実例: retentionDays: 0 を名前から「保持しない」と読んだが、正しくは「無期限」だった。
   また「効いていない」と言うときは、**値が空だから**なのか
   **そもそも読む側が存在しないから**なのかを区別して書く。
4c. 自分の解釈と観測が矛盾したら（例:「履歴は残らない設定」なのに履歴が2件ある）、
   観測を優先し、**解釈が誤っている可能性を述べる**。
   辻褄合わせの説明（「セッション内では保持されている」等）を作らない。
...
7. Markdown で書く。表・箇条書きを使ってよい。見出しは ### 以下にする。
   **表の直前には必ず空行を入れる。** 文の直後に続けて書くと表として描画されず、
   パイプ記号の行が生のまま画面に出る。
${instructions.trim() ? `
