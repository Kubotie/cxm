...
import { fetchCompanyByUid, fetchAlerts, fetchEvidence, fetchPeople } from "@/lib/nocodb-client";
import type { AppCompany, AppAlert, AppEvidence, AppPerson } from "@/lib/nocodb-client";
...
import { Building, Users, AlertTriangle, CheckSquare, Clock, ExternalLink, Sparkles, Network, ArrowRight, Send, Database, FileText, BookOpen, Calendar, Headphones, Loader2, RefreshCw, ShieldAlert, TrendingUp, Zap } from "lucide-react";
...
export function CompanyDetail() {
  ...
  const [apiEvidence,  setApiEvidence]  = useState<AppEvidence[]| null>(null);
  const [apiPeople,    setApiPeople]    = useState<AppPerson[]  | null>(null);
  ...
  useEffect(() => {
    ...
    fetchCompanyByUid(companyId)
      ...
      .catch(e => {
        console.warn('[CompanyDetail] company fetch failed:', e);
        setLoadError('会社情報の取得に失敗しました（mock表示中）');
      });

    fetchAlerts(companyId)
      .then(setApiAlerts)
      .catch(e => console.warn('[CompanyDetail] alerts fetch failed:', e));

    fetchEvidence(companyId)
      .then(setApiEvidence)
      .catch(e => console.warn('[CompanyDetail] evidence fetch failed:', e));

    fetchPeople(companyId)
      .then(setApiPeople)
      .catch(e => console.warn('[CompanyDetail] people fetch failed:', e));
  }, [companyId]);

  // API データがあればそちら、なければ mock にフォールバック
  const company  = apiCompany
    ? { ...mockCompany, id: apiCompany.id, name: apiCompany.name, phaseLabel: apiCompany.phaseLabel,
        openAlerts: apiCompany.openAlerts, openActions: apiCompany.openActions,
        lastContact: apiCompany.lastContact }
    : { ...mockCompany, id: companyId ?? '1' };
  const alerts   = apiAlerts   ?? mockAlerts;
  const evidence = apiEvidence ?? mockEvidence;
  const people   = apiPeople   ?? mockPeople;
  ...
  async function generateCompanySummary() {
    ...
    try {
      ...
      const data = await res.json() as CompanyEvidenceSummaryApiResponse;
    ...
    <div className="flex h-screen bg-slate-50">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                    ...
                    <h1 className="font-semibold text-slate-900">{company.name}</h1>
                  </div>
                  ...
                  <div className="flex items-center gap-3 text-xs text-slate-600">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="font-semibold text-red-600">{company.openAlerts}</span>
                      <span>Alerts</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <CheckSquare className="w-3 h-3 text-blue-500" />
                      <span className="font-semibold text-blue-600">{company.openActions}</span>
                      <span>Actions</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <Clock className="w-3 h-3 text-slate-400" />
                      <span>最終接点: {company.lastContact}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      ...
                      {company.projects.map((project, idx) => (
                        <Link 
                          key={idx}
                          href={`/project/proj_${idx + 1}`}
                          className="text-blue-600 hover:text-blue-800 hover:underline font-medium"
                        >
                          {project}{idx < company.projects.length - 1 ? ',' : ''}
                        </Link>
                      ))}
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-2">
                  <Link href={`/outbound/compose?fromCompany=${companyId}`}>
                ...
                <div className="p-4 border-b bg-slate-50 flex-shrink-0">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <Badge variant="outline" className="text-xs bg-amber-100">
                      未処理: {evidence.filter(e => e.status === "unprocessed").length}件
                    </Badge>
                  </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                  ...
                  <div className="mb-6">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Badge className="text-xs h-5 bg-red-500">{alerts.length}</Badge>
                    </div>
                    <div className="space-y-2">
                      {alerts.map((alert) => (
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className="text-xs bg-emerald-100">
                        確定: {people.filter(p => p.status === "confirmed").length}名
                      </Badge>
                      <Badge variant="outline" className="text-xs bg-blue-100">
                        候補: {people.filter(p => p.status === "proposed").length}名
                      </Badge>
                    </div>
                  </div>
                </div>
                  ...
                  <Tabs defaultValue="org" className="h-full flex flex-col">
                    ...
                    <TabsContent value="people" className="flex-1 overflow-y-auto mx-4 mt-4 space-y-4">
                      ...
                      <div className="mb-6">
                        ...
                        <div className="space-y-3">
                          {people.filter(p => p.status === "confirmed").map((person) => (
                      ...
                      <div className="mb-4">
                        ...
                        <div className="space-y-3">
                          {people.filter(p => p.status === "proposed").map((person) => (
                            <PeopleCard key={person.id} {...person} />
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="mb-4">
                        ...
                        <div className="space-y-3">
                          {people.filter(p => p.status === "unresolved").map((person) => (
                            <PeopleCard key={person.id} {...person} />
                          ))}
                        </div>
                      </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        <div className="grid grid-cols-2 gap-2">
                          {company.projects.map((project, idx) => (
                            <Link key={idx} href={`/project/proj_${idx + 1}`}>
                        ...
                        {/* 主CTA: Company施策導線 */}
                        <div className="space-y-2">
                          <Link href={`/outbound/compose?sourceContext=company&linkedCompany=${company.id}&companyName=${encodeURIComponent(company.name)}&phase=${company.phaseLabel}`}>
                        ...
                        {/* 補助CTA: 業務文脈へ */}
                        <div className="space-y-2">
                          <Link href={`/actions?context=company&id=${company.id}`}>
                            <Button variant="outline" size="sm" className="w-full text-xs h-8 justify-start">
                              <CheckSquare className="w-3 h-3 mr-2" />
                              Actionsに送る
                            </Button>
                          </Link>
                          <Link href={`/content?context=company&id=${company.id}`}>
                            <Button variant="outline" size="sm" className="w-full text-xs h-8 justify-start">
                        ...
                        <div>
                          <Button
                            variant="outline"
                            size="sm"
                            className="w-full text-xs h-8 justify-start border-violet-200 text-violet-700 hover:bg-violet-50"
                          ...
                          >
                            ...
                            {summaryResult && !summaryLoading && (
                              <RefreshCw className="w-3 h-3 ml-auto text-violet-400" />
                            )}
                          </Button>
                        </div>
                        ...
                        {/* 補助CTA: 参照 */}
                        <div className="space-y-2">
                          <Link href={`/unified-log?company=${company.id}`}>
                            <Button variant="outline" size="sm" className="w-full text-xs h-8 justify-start">
      ...
      <Sheet open={showSalesforceSyncSheet} onOpenChange={setShowSalesforceSyncSheet}>
        <SheetContent className="w-[700px] sm:max-w-[700px]">
            ...
            <div className="space-y-4 px-4">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                ...
                <div className="space-y-1 text-xs text-blue-800">
                  <p>• Salesforce Account: {company.name}</p>
          ...
          <div className="flex gap-2 mt-4 px-4">
            <Link 
                          href={`/outbound/compose?fromCompany=${company.id}&sync=salesforce&type=org_chart`}
              className="flex-1"
            >
              <Button
                variant="default"
      ...
      <Sheet open={showSummarySheet} onOpenChange={setShowSummarySheet}>
        <SheetContent className="w-[560px] sm:max-w-[560px]">
          <SheetHeader>
            ...
            <SheetDescription>
              Evidence / Alerts / People を総合した AI 分析
            </SheetDescription>
          </SheetHeader>
            ...
            <div className="space-y-4 px-1 pr-4">
              ...
              {(summaryLoading || summarySaving) && (
                <div className="flex flex-col items-center justify-center py-16 gap-3 text-slate-500">
                  ...
                  <p className="text-sm">
                    {summarySaving ? '保存しています...' : 'Evidence を分析しています...'}
                  </p>
                </div>
              )}
              ...
              {!summaryLoading && !summarySaving && summaryError && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-4 space-y-2">
                  <p className="text-sm font-semibold text-red-800">失敗しました</p>
                  <p className="text-xs text-red-700">{summaryError}</p>
                  <Button variant="outline" size="sm" className="text-xs mt-2" onClick={generateCompanySummary}>
              ...
              {/* ─ Result ─ */}
              {!summaryLoading && !summarySaving && summaryResult && (
                <>
                  {/* Meta */}
                  <div className="flex items-center gap-2 flex-wrap">
                    {/* Health badge */}
                    {summaryResult.overall_health === 'healthy' && (
                      <Badge className="bg-emerald-100 text-emerald-800 border-emerald-200">健全</Badge>
                    )}
                    {summaryResult.overall_health === 'at_risk' && (
                      <Badge className="bg-amber-100 text-amber-800 border-amber-200">要注意</Badge>
                    )}
                    {summaryResult.overall_health === 'critical' && (
                      <Badge className="bg-red-100 text-red-800 border-red-200">要緊急対応</Badge>
                    )}
                    {summaryResult.overall_health === 'expanding' && (
                      <Badge className="bg-blue-100 text-blue-800 border-blue-200">拡大機会</Badge>
                    )}
                    {summaryResult.saved === true && (
                      <Badge className="bg-violet-100 text-violet-700 border-violet-200 text-xs">
                        {summaryResult.created ? '新規保存' : '上書き保存'}
                      </Badge>
                    )}
                    <span className="text-xs text-slate-400">
                      Evidence {summaryResult.evidence_count}件 · Alerts {summaryResult.alert_count}件 · People {summaryResult.people_count}名
                    </span>
                    <span className="text-xs text-slate-400 ml-auto">{summaryResult.model}</span>
                  </div>
                  {summaryResult.save_error && (
                    <p className="text-xs text-amber-600">{summaryResult.save_error}</p>
                  )}
                  ...
                  {/* Summary */}
                  <div>
                    <h3 className="text-xs font-semibold text-slate-700 mb-2 uppercase tracking-wide">Summary</h3>
                    <p className="text-sm text-slate-800 leading-relaxed">{summaryResult.summary}</p>
                  </div>

                  {/* Key Risks */}
                  {summaryResult.key_risks.length > 0 && (
                    <div>
                      <h3 className="text-xs font-semibold text-slate-700 mb-2 flex items-center gap-1 uppercase tracking-wide">
                        <ShieldAlert className="w-3 h-3 text-red-500" />
                        Key Risks
                      </h3>
                      <div className="space-y-2">
                        {summaryResult.key_risks.map((risk, i) => (
                          <div key={i} className="bg-red-50 border border-red-100 rounded-lg p-3">
                            <p className="text-xs font-semibold text-red-800 mb-0.5">{risk.title}</p>
                  ...
                  {/* Key Opportunities */}
                  {summaryResult.key_opportunities.length > 0 && (
                    <div>
                      <h3 className="text-xs font-semibold text-slate-700 mb-2 flex items-center gap-1 uppercase tracking-wide">
                      ...
                      <div className="space-y-2">
                        {summaryResult.key_opportunities.map((opp, i) => (
                          <div key={i} className="bg-emerald-50 border border-emerald-100 rounded-lg p-3">
                            <p className="text-xs font-semibold text-emerald-800 mb-0.5">{opp.title}</p>
                            <p className="text-xs text-emerald-700">{opp.description}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {/* No risks / no opportunities fallback */}
                  {summaryResult.key_risks.length === 0 && summaryResult.key_opportunities.length === 0 && (
                    <div className="text-xs text-slate-400 text-center py-2">
                      リスク・オポチュニティは検出されませんでした
                    </div>
                  )}
              ...
              {/* ─ Empty (Sheet opened but no result yet — shouldn't happen normally) ─ */}
              {!summaryLoading && !summarySaving && !summaryResult && !summaryError && (
