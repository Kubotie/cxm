...
import type { UnifiedLogSignalApiResponse } from "@/lib/prompts/unified-log-signal";
...
export function UnifiedCompanyLog() {
  ...
  const [signalResult,     setSignalResult]     = useState<UnifiedLogSignalApiResponse | null>(null);
  const [signalError,      setSignalError]      = useState<string | null>(null);

  async function extractSignals(save: boolean) {
    if (save) { setSignalSaving(true); } else { setSignalLoading(true); }
    setSignalError(null);
    setShowSignalSheet(true);
    try {
      const res = await fetch(`/api/companies/${companyId}/log/signals`, {
        method: 'POST',
      ...
      const data = await res.json() as UnifiedLogSignalApiResponse;
          ...
          <aside className="w-64 border-r bg-white flex flex-col">
            ...
            <div className="p-3 border-t space-y-1.5">
              ...
              <Button
                ...
                onClick={() => extractSignals(false)}
                disabled={signalLoading || signalSaving}
              >
                {signalLoading
                  ? <Loader2 className="w-3 h-3 mr-2 animate-spin" />
                  : <Zap className="w-3 h-3 mr-2" />
                }
                {signalLoading ? '抽出中...' : 'Extract Only'}
              </Button>
              {/* Extract and Save */}
              <Button
                variant="outline"
                size="sm"
                className="w-full text-xs h-8 justify-start border-violet-300 bg-violet-50 text-violet-800 hover:bg-violet-100"
                onClick={() => extractSignals(true)}
                disabled={signalLoading || signalSaving}
              >
                {signalSaving
                  ? <Loader2 className="w-3 h-3 mr-2 animate-spin" />
                ...
                {signalSaving ? '保存中...' : 'Extract and Save'}
              </Button>
              {signalResult && !signalLoading && !signalSaving && (
                <button
                  onClick={() => setShowSignalSheet(true)}
                  className="w-full mt-0.5 text-xs text-violet-600 hover:text-violet-800 text-center"
                >
                  結果を見る →
                </button>
              )}
      ...
      <Sheet open={showSignalSheet} onOpenChange={setShowSignalSheet}>
        <SheetContent className="w-[560px] sm:max-w-[560px]">
          <SheetHeader>
            ...
            <SheetDescription>
              Unified Log からリスク・オポチュニティ・不足情報を抽出
            </SheetDescription>
          </SheetHeader>
            ...
            <div className="space-y-4 px-1 pr-4">
              ...
              {(signalLoading || signalSaving) && (
                <div className="flex flex-col items-center justify-center py-16 gap-3 text-slate-500">
                  ...
                  <p className="text-sm">{signalSaving ? 'ログを分析して保存しています...' : 'ログを分析しています...'}</p>
                </div>
              )}
              ...
              {!signalLoading && signalError && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-4 space-y-2">
                  <p className="text-sm font-semibold text-red-800">抽出に失敗しました</p>
                  <p className="text-xs text-red-700">{signalError}</p>
                  <Button
                    variant="outline"
                    size="sm"
              ...
              {/* ─ Result ─ */}
              {!signalLoading && signalResult && (
                <>
                  {/* Meta */}
                  <div className="flex items-center gap-2 text-xs text-slate-400 flex-wrap">
                    <span>ログ {signalResult.log_count}件を分析</span>
                    <span>·</span>
                    <span>{signalResult.model}</span>
                    {signalResult.saved === true && (
                      <Badge className="bg-emerald-100 text-emerald-800 border-emerald-200 text-xs">
                        {signalResult.created ? '新規保存' : '上書き保存'}
                      </Badge>
                    )}
                    {signalResult.save_error && (
                      <Badge className="bg-amber-100 text-amber-800 border-amber-200 text-xs">
                        保存失敗
                      </Badge>
                    )}
                    <span className="ml-auto">
                      {new Date(signalResult.generated_at).toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit' })}
                    </span>
                  </div>
                  {signalResult.save_error && (
                    <div className="bg-amber-50 border border-amber-200 rounded p-2 text-xs text-amber-800">
                      保存エラー: {signalResult.save_error}
                    </div>
                  )}
                  ...
                  {/* Key Signals */}
                  {signalResult.key_signals.length > 0 && (
                    <div>
                      ...
                      <div className="space-y-2">
                        {signalResult.key_signals.map((s, i) => (
                          <div
                            key={i}
                          ...
                          >
                            <div className="flex items-center gap-1.5 mb-0.5">
                              {s.signal_type === 'risk'        && <ShieldAlert className="w-3 h-3 text-red-500 flex-shrink-0" />}
                              {s.signal_type === 'opportunity' && <TrendingUp  className="w-3 h-3 text-emerald-500 flex-shrink-0" />}
                              {s.signal_type === 'info'        && <Info        className="w-3 h-3 text-slate-400 flex-shrink-0" />}
                              <p className={`text-xs font-semibold ${
                                s.signal_type === 'risk' ? 'text-red-800'
                                : s.signal_type === 'opportunity' ? 'text-emerald-800'
                                : 'text-slate-700'
                              }`}>{s.title}</p>
                            </div>
                            <p className={`text-xs ${
                              s.signal_type === 'risk' ? 'text-red-700'
                              : s.signal_type === 'opportunity' ? 'text-emerald-700'
                              : 'text-slate-600'
                            }`}>{s.description}</p>
                          </div>
                        ))}
                  ...
                  {/* Risk Signals */}
                  {signalResult.risk_signals.length > 0 && (
                    <div>
                      <h3 className="text-xs font-semibold text-slate-700 mb-2 flex items-center gap-1 uppercase tracking-wide">
                      ...
                      <div className="space-y-2">
                        {signalResult.risk_signals.map((r, i) => (
                          <div key={i} className="bg-red-50 border border-red-100 rounded-lg p-3">
                            <div className="flex items-start justify-between gap-2 mb-0.5">
                              <p className="text-xs font-semibold text-red-800">{r.title}</p>
                              <Badge className={`text-xs flex-shrink-0 ${
                                r.urgency === 'high'   ? 'bg-red-200 text-red-900'
                                : r.urgency === 'medium' ? 'bg-amber-100 text-amber-900'
                  ...
                  {/* Opportunity Signals */}
                  {signalResult.opportunity_signals.length > 0 && (
                    <div>
                      <h3 className="text-xs font-semibold text-slate-700 mb-2 flex items-center gap-1 uppercase tracking-wide">
                      ...
                      <div className="space-y-2">
                        {signalResult.opportunity_signals.map((o, i) => (
                          <div key={i} className="bg-emerald-50 border border-emerald-100 rounded-lg p-3">
                            <div className="flex items-start justify-between gap-2 mb-0.5">
                              <p className="text-xs font-semibold text-emerald-800">{o.title}</p>
                              <Badge className={`text-xs flex-shrink-0 ${
                                o.potential_impact === 'high'   ? 'bg-emerald-200 text-emerald-900'
                                : o.potential_impact === 'medium' ? 'bg-teal-100 text-teal-900'
                                : 'bg-slate-100 text-slate-700'
                              }`}>
                                {o.potential_impact}
                              </Badge>
                  ...
                  {/* Missing Information */}
                  {signalResult.missing_information.length > 0 && (
                    <div>
                      <h3 className="text-xs font-semibold text-slate-700 mb-2 flex items-center gap-1 uppercase tracking-wide">
                      ...
                      <div className="space-y-2">
                        {signalResult.missing_information.map((m, i) => (
                          <div key={i} className="bg-amber-50 border border-amber-100 rounded-lg p-3">
                            <p className="text-xs font-semibold text-amber-800 mb-0.5">{m.category}</p>
                            <p className="text-xs text-amber-700">{m.description}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="bg-violet-50 border border-violet-200 rounded-lg p-3">
                      <p className="text-sm text-violet-900">{signalResult.recommended_next_review_focus}</p>
                    </div>
                  </div>
              ...
              {/* ─ Empty ─ */}
              {!signalLoading && !signalResult && !signalError && (
                <div className="text-center py-12 text-xs text-slate-400">
                  <Zap className="w-8 h-8 mx-auto mb-3 text-slate-300" />
                  <p>ボタンを押すとシグナルの抽出が始まります</p>
                </div>
              )}
