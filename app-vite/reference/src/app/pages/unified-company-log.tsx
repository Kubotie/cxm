...
export function UnifiedCompanyLog() {
  ...
  const selectedLogData = mockLogs.find(log => log.id === selectedLog);
  ...
  const unresolvedCount = mockLogs.filter(log => log.status === "unresolved" || log.resolverResult === "unresolved").length;
  const unprocessedCount = mockLogs.filter(log => log.status === "unprocessed").length;
  ...
  return (
    <div className="flex h-screen bg-slate-50">
            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-1">
                  <h1 className="text-lg font-semibold text-slate-900">{companyData.name}</h1>
                  <Badge className="bg-blue-500 text-xs">{companyData.phaseLabel}</Badge>
                  {companyData.healthTrend === "down" && (
                    <div className="flex items-center gap-1 text-orange-700">
                      <TrendingDown className="w-4 h-4" />
                      <span className="text-xs font-medium">Health Score {companyData.healthScore}</span>
                    </div>
                  )}
                </div>
                <div className="flex items-center gap-4 text-xs text-slate-600">
                  <span>Owner: {companyData.accountOwner}</span>
                  <span>•</span>
                  <span>最終接点: {companyData.lastTouchpoint}</span>
                  <span>•</span>
                  <span>関連案件: {companyData.relatedProjects.join(", ")}</span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-5 gap-3">
              <div className="bg-red-50 border border-red-200 rounded px-3 py-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-lg font-bold text-red-700">{companyData.openAlerts}</span>
                </div>
              </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded px-3 py-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-lg font-bold text-blue-700">{companyData.openActions}</span>
                </div>
              </div>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded px-3 py-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-lg font-bold text-slate-700">{mockLogs.length}</span>
                </div>
              </div>
            </div>
          </div>
                      ...
                      .map(([date, logs]) => (
                        <div key={date}>
                          ...
                          <div className="space-y-2 ml-4">
                            {logs.map((log) => {
                              const sourceConfig = sourceTypeConfig[log.sourceType];
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <SheetDescription className="text-xs mt-1">
                      {selectedLogData.timestamp} • {sourceTypeConfig[selectedLogData.sourceType].label}
                    </SheetDescription>
                  </div>
                </div>
              </SheetHeader>
              ...
              <ScrollArea className="h-[calc(100vh-80px)]">
                <div className="p-4 space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Badge variant="outline" className={`text-xs ${statusConfig[selectedLogData.status].color}`}>
                      {statusConfig[selectedLogData.status].label}
                    </Badge>
                      ...
                      <table className="w-full text-xs">
                        <tbody>
                          <tr className="border-b">
                            ...
                            <td className="p-2">{sourceTypeConfig[selectedLogData.sourceType].label}</td>
                          </tr>
                          <tr className="border-b">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      {(selectedLogData.messageType === 'inquiry' || 
                        ...
                        <Link to={`/outbound/compose?fromLog=${selectedLogData.id}&evidence=${selectedLogData.id}&sourceContext=unified_log&linkedCompany=${companyData.id}&linkedProject=${selectedLogData.linkedProject || ''}&summary=${encodeURIComponent(selectedLogData.summary)}`}>
                          <Button 
                            variant="default" 
                            size="sm" 
      ...
      <LogActionPanels
        ...
        relatedProjects={companyData.relatedProjects}
