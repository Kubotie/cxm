...
const supportCases = [
  ...
  },
  {
    ...
    sourceStatus: null, // Slack has no source status
    severity: "medium",
  ...
  },
  {
    ...
    sourceStatus: null, // Chatwork has no source status
    severity: "medium",
...
export function SupportQueue() {
  ...
  const viewConfig: Record<string, {
    subtitle: string;
    filterCases: (c: typeof supportCases[0]) => boolean;
    ...
    all: {
      ...
      emptyState: {
        title: "案件がありません",
        description: "問い合わせやサポート案件が発生すると、ここに表示されます",
        cta: null,
      },
    },
    unassigned: {
      ...
      emptyState: {
        title: "未アサインの案件はありません",
        description: "すべての案件が振り分け済みです",
        cta: { label: "All Cases を見る", action: () => setCurrentView("all") },
      },
    },
    my: {
      subtitle: "自分が担当している案件を確認する",
      filterCases: (c) => c.owner === "山本 一郎", // Mock
      emptyState: {
        title: "担当している案件はありません",
        description: "案件が割り当てられると、ここに表示されます",
        cta: { label: "Unassigned を見る", action: () => setCurrentView("unassigned") },
      },
    },
    inquiry: {
      ...
      emptyState: {
        title: "問い合わせはありません",
        description: "新規問い合わせが発生すると、ここに表示されます",
        cta: { label: "All Cases を見る", action: () => setCurrentView("all") },
      },
    },
    support: {
      ...
      emptyState: {
        title: "サポート案件はありません",
        description: "サポート案件が発生すると、ここに表示されます",
        cta: { label: "All Cases を見る", action: () => setCurrentView("all") },
      },
    },
    cse: {
      ...
      emptyState: {
        title: "CSE連携案件はありません",
        description: "CSE Ticketと連携した案件が発生すると、ここに表示されます",
        cta: { label: "All Cases を見る", action: () => setCurrentView("all") },
      },
    },
  };
  ...
  if (caseTypeFilter !== "all") {
    filteredCases = filteredCases.filter(c => c.caseType === caseTypeFilter);
  }

  if (sourceFilter !== "all") {
    filteredCases = filteredCases.filter(c => c.source === sourceFilter);
  }

  if (statusFilter !== "all") {
    filteredCases = filteredCases.filter(c => c.routingStatus === statusFilter);
  }
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredCases.map((supportCase) => (
                      <TableRow key={supportCase.id} className="hover:bg-slate-50">
                        <TableCell>
                          <Link 
                        ...
                        <TableCell>
                          <div className="space-y-1">
                            <Link 
                              to={`/company/${supportCase.companyId}`}
                              className="text-sm text-slate-700 hover:text-blue-600 hover:underline block"
                            >
                            ...
                            {supportCase.project && (
                              <Link 
                                to={`/project/${supportCase.projectId}`}
                                className="text-xs text-slate-600 hover:text-blue-600 hover:underline block"
                              >
                                {supportCase.project}
                              </Link>
                            )}
                        ...
                        <TableCell className="text-sm text-slate-700">
                          {supportCase.owner || <span className="text-slate-400">-</span>}
                        </TableCell>
                        <TableCell>
                          {supportCase.assignedTeam ? (
                            <Badge variant="outline" className="text-xs">
                              {supportCase.assignedTeam}
                            </Badge>
                          ) : (
                        ...
                        <TableCell>
                          {supportCase.routingStatus === "unassigned" && (
                            <Badge variant="outline" className="bg-slate-100 text-slate-700 text-xs">
                              未アサイン
                            </Badge>
                          )}
                          {supportCase.routingStatus === "triaged" && (
                            <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200 text-xs">
                              振り分け済
                            </Badge>
                          )}
                          {supportCase.routingStatus === "assigned" && (
                            <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-200 text-xs">
                              アサイン済
                            </Badge>
                          )}
                          {supportCase.routingStatus === "in progress" && (
                            <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 text-xs">
                              対応中
                            </Badge>
                          )}
                          {supportCase.routingStatus === "waiting on customer" && (
                          ...
                          {supportCase.routingStatus === "resolved_like" && (
                            <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 text-xs">
                              区切り済み
                            </Badge>
                          )}
                        ...
                        <TableCell className="text-xs text-slate-600">
                          {supportCase.sourceStatus || <span className="text-slate-400">-</span>}
                        </TableCell>
                        <TableCell>
                          ...
                          {supportCase.severity === "medium" && (
                            <Badge className="bg-amber-600 text-white text-xs">Medium</Badge>
                          )}
                          {supportCase.severity === "low" && (
                            <Badge className="bg-slate-600 text-white text-xs">Low</Badge>
                          )}
                        </TableCell>
                        <TableCell className="text-xs text-slate-600">
                        ...
                        <TableCell className="text-xs text-slate-600">
                          {supportCase.firstResponseTime || <span className="text-slate-400">-</span>}
                        </TableCell>
                        <TableCell className="text-xs text-slate-600">
                          {supportCase.openDuration}
                          {supportCase.waitingDuration && (
                            <div className="text-xs text-amber-600 mt-0.5">
                              待機: {supportCase.waitingDuration}
                            </div>
                          )}
                        </TableCell>
                        <TableCell>
                          <div className="flex flex-col gap-1">
                            {supportCase.linkedCSETicket && (
                              <Badge variant="outline" className="text-xs bg-amber-50 border-amber-200 text-amber-700">
                                <Ticket className="w-3 h-3 mr-1" />
                                {supportCase.linkedCSETicket}
                              </Badge>
                            )}
                            {supportCase.relatedContent > 0 && (
                              <Badge variant="outline" className="text-xs bg-blue-50 border-blue-200 text-blue-700">
                                Content: {supportCase.relatedContent}
                              </Badge>
                            )}
                          </div>
                        </TableCell>
                        <TableCell>
                          <Link to={`/support/${supportCase.id}`}>
